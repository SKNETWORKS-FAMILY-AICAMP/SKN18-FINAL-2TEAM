    ...
    try:
        ...
        system_prompt = SystemMessage(
            content=(
                ...
                "AI 응답 내용을 분석하여 다음 중 적절한 방식을 선택해라:\n"
                "1. 시간적/단계적 흐름이 명확한 경우: 기→승→전→결 구조로 순차적 flowchart 생성 (예: 연구 배경 → 가설 설정 → 실험 수행 → 결과 분석)\n"
                "2. 개념 간 관계가 중심인 경우: 핵심 개념과 인과/연관 관계를 중심으로 flowchart 생성 (예: BRCA1 돌연변이 → DNA 복구 실패 → 유방암 위험 증가)\n"
                "꼭 'graph LR'로 시작하되, 한눈에 흐름이 보이도록 최대한 직관적으로 작성한다. "
                "각 노드는 주요 개념(명사, 주제어 등)만 사용하고, 의미 없는 부연 설명이나 장황한 문장은 노드로 만들지 않는다. "
                "모든 엣지는 실제로 언급된 '원인→결과', '주제→속성', '단계→다음단계' 식의 관계만 나타내라. "
