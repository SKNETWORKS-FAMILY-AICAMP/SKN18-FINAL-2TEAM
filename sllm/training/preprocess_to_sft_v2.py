...
def create_annotation_json():
    ...
    annotation_output = rf"c:\dev\ai_camp\SKN18-FINAL-2TEAM\sllm\datasets\annotation_v2_{timestamp}.json"
...
def create_sft_dataset(annotation_output):
    ...
    # 타임스탬프 생성 (YYMMDDHHMMSS 형식)
    timestamp = datetime.now().strftime("%y%m%d%H%M%S")
    sft_output = rf"c:\dev\ai_camp\SKN18-FINAL-2TEAM\sllm\datasets\sft_dataset_v2_{timestamp}.jsonl"
