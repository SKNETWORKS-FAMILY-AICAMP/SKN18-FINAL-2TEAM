...
class GraphSchemaQueries:
    ...
    # 설명: 데이터 무결성 보장 및 Exact Match 검색 속도 최적화
    CREATE_CONSTRAINTS = [
        # [PrimeKG] BaseNode
        ...
        # [Protocol] ★ 중요: Property Name 변경됨 (chunk_id -> chunking_id)
    ...
    # 설명: Semantic Search용. 검색 쿼리 이름과 코드상 차원 설정(1536/1024) 주의
    CREATE_VECTOR_INDEXES = [
        # [Paper Chunk] 차원 1536 (OpenAI), 이름은 검색 쿼리와 일치시킴
        """
        ...
        """,
        # [Protocol Chunk] 차원 1024 (BGE-M3 등)
        """
...
class GraphSearchQueries:
    ...
    # ==========================================================================
    # 1. Basic Operators (Fulltext & Vector Seed Generation)
    # ==========================================================================
    ...
    # [OP] FT_MENTION_RAW
    # 수정: Mention은 'raw_text' 필드를 사용하므로 인덱스 설정과 일치
    FT_MENTION_RAW = """
    ...
    # [OP] VEC_PAPER_CHUNK
    # 인덱스: articleChunkEmbeddingIndex (Dim: 1536)
    VEC_PAPER_CHUNK = """
    ...
    # [OP] HY_PAPER_CHUNK_VEC_PLUS_MUST (Vector + Keyword Filtering)
    ...
    # [OP] HY_PAPER_CHUNK_DUAL_RRF (Vector + Fulltext + RRF)
    ...
    YIELD node AS c, score AS vec_score
    WITH c, vec_score ORDER BY vec_score DESC
    ...
    WITH c, ft_score ORDER BY ft_score DESC
    ...
    # 수정: chunk_id -> chunking_id / 인덱스: protocolChunkEmbeddingIndex (Dim: 1024)
    VEC_PROTOCOL_CHUNK = """
    ...
    # [OP] HY_PROTOCOL_CHUNK_DUAL_RRF
    # 수정: 서브쿼리 및 매칭에서 chunk_id -> chunking_id
    HY_PROTOCOL_CHUNK_DUAL_RRF = """
    CALL {
    WITH $embedding AS embedding, coalesce($fetch_vec, 150) AS fetch_vec, coalesce($k_vec, 80) AS k_vec
    CALL db.index.vector.queryNodes('protocolChunkEmbeddingIndex', fetch_vec, embedding)
    YIELD node AS pc, score AS vec_score
    WITH pc, vec_score ORDER BY vec_score DESC
      // chunking_id를 chunk_id로 alias하여 RRF 계산 로직 재사용
    ...
    WITH pc, ft_score ORDER BY ft_score DESC
    ...
    WITH cc, ft_score ORDER BY ft_score DESC
    ...
    # [OP] FT_TRIAL_ID (Exact Match or Index Seek)
    FT_TRIAL_ID = """
...
class GraphCellQueries:
    ...
    - Entity(T1), Vector(T2), Graph(T3), Multi-domain(T4) 전략 포함
    - queries_v2.py 스키마(ProtocolChunk.chunking_id 등) 반영됨
    """
    ...
    # [PAPER T1] Entity ID 기반 검색
    # 수정: m.text -> m.raw_text, j.title -> j.name
    PAPER_T1 = """
    ...
    coalesce(a.pagerank, 0.0) AS pagerank,
      j.name AS journal_title,  // [수정] j.title -> j.name
    evidence_mentions,
    evidence_chunks
    ORDER BY mention_cnt DESC, year DESC
    """

    # [PAPER T2] Chunk ID (Vector Search Result) 기반 검색
    # 수정: j.title -> j.name
    PAPER_T2 = """
    ...
    # [PAPER T3] Entity -> Article -> Citation 확장 검색
    PAPER_T3 = """
    WITH $entity_ids AS entity_ids
    MATCH (e:Entity)
    WHERE e.entity_id IN entity_ids
    ...
    # [PROTOCOL T1] Entity -> Article -> Experiment -> Protocol
    # 수정: pc.chunk_id -> pc.chunking_id
    PROTOCOL_T1 = """
    WITH $entity_ids AS entity_ids
    MATCH (e:Entity)
    WHERE e.entity_id IN entity_ids
    ...
    # [PROTOCOL T2] Vector Search Result 기반
    # 수정: WHERE pc.chunking_id, $chunk_score_by_id[pc.chunking_id]
    PROTOCOL_T2 = """
    ...
    # [PROTOCOL T3] Entity -> Experiment -> Protocol
    PROTOCOL_T3 = """
    WITH $entity_ids AS entity_ids
    MATCH (e:Entity)
    WHERE e.entity_id IN entity_ids
